...
import static hu.edu.greenleaves.bank.servlet.ServletPaths.APPLY_LOAN;
...
public class ApplyLoanServlet extends DefaultServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			clientTransaction.setLoanAmount(amount);
			...
			if (transactionCodesDAO.validCode(code,clientInfo.getUser().getId()) && clientTransactionDAO.validLoanTransaction(clientTransaction)) {
				transactionCodesDAO.updateUsage(code, clientInfo.getUser().getId());
				clientTransactionDAO.createLoan(clientTransaction);
